...
系统已经实现以下主流程：协议发布后不可原地覆盖；批次按版本从草稿进入运行、封存、分析和决定状态；观测分片同时受请求幂等键和来源行唯一身份保护；排除请求必须由不同角色复核；分析任务使用 SQLite 租约避免重复执行并支持过期接管：每次领取都会签发单调递增的领取代次 `lease_generation` 作为持有凭证（fencing token），完成任务时必须出示该凭证；写回的同一个 `BEGIN IMMEDIATE` 事务会重新校验任务状态、持有人、领取代次、租约有效期和批次状态/代次，失去租约的旧工作进程即使已经算出结果也无法新增分析、推进批次、完成任务或留下成功审计，其陈旧完成请求只会进入 `job_events` 拒绝台账。同一输入快照使用固定算法版本和随机种子得到一致结果，合法持有人重放完成请求返回同一分析记录；分析者与审批人职责分离，报告保留输入摘要、统计规则和批次审计链。
